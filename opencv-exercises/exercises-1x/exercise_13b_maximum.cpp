...
bool is_maximum_flat_zone(cv::Mat img, cv::Mat &output_image, int x, int y, int value, int label, CONNECT_TYPE con)
{
    // out of limits
    ...
    bool is_maximum = 
    is_maximum_flat_zone(img, output_image, x-1, y, value, label, con) &&
    is_maximum_flat_zone(img, output_image, x+1, y, value, label, con) &&
    is_maximum_flat_zone(img, output_image, x, y-1, value, label, con) &&
    is_maximum_flat_zone(img, output_image, x, y+1, value, label, con);

    if (con == CONNECT_8)
    {
        is_maximum = is_maximum &&
        is_maximum_flat_zone(img, output_image, x-1, y-1, value, label, con) &&
        is_maximum_flat_zone(img, output_image, x+1, y-1, value, label, con) &&
        is_maximum_flat_zone(img, output_image, x-1, y+1, value, label, con) &&
        is_maximum_flat_zone(img, output_image, x+1, y+1, value, label, con);
    }
...
int main(int argc, char** argv )
{
    if (argc != 4)
    {
        printf("usage: ./exercise_13b_maximum <input_txt_path> <input_image_path> <output_image_path>\n");
        return -1;
    }
    ...
    bool is_maximum = is_maximum_flat_zone(input_image, output_image, x, y, input_image.at<uchar>(x,y), label, connectivity);
